...
def input_data():
    ...
    while indeling_correct == False:
        cfg.breedte_kast=float(input('Wat is de breedte van de kast in cm? '))
        voetjes=str(raw_input('heeft de kast pootjes? [y/n] '))
        if voetjes == ('ja' or 'JA') or ('Ja' or 'YES') or ('yes' or 'Yes') or ('j' or 'y'):
            cfg.hoogte_voet=float(input('Hoe hoog zijn de pootjes in cm? '))
        else:
            cfg.hoogte_voet = 0.
        cfg.hoogte_kast=float(input('Wat is de hoogte van de kast in cm? '))
        cfg.hoogte_kast=cfg.hoogte_kast-cfg.hoogte_voet
        cfg.diepte_kast=float(input('Wat is de diepte van de kast in cm? '))
        cfg.breedte_plank=float(input('Wat is de breedte van de plank in cm? '))
        cfg.lengte_plank=float(input('Wat is de lengte van de plank in cm? '))
        cfg.dikte_plank=float(input('Wat is de dikte van de plank in cm? '))
        cfg.niveaus=int(input('Hoeveel niveaus wil je ? '))
    ...
    while indeling_correct == False:
        for niveau in range(cfg.niveaus):
            ...
            if niveau == 0:
                afstand=int(input('Wat is de afstand van onderplank tot plank %d in cm? ' % (niveau+1)))
                ...
                cfg.afstand=float(input('Wat is de afstand tussen plank %d en plank %d in cm? ' % (niveau,niveau+1)))
                if afstand != 0:  
        ...
        check=raw_input('Ben je tevreden met deze waarden ? [Ja/Nee] ')
