...
def get_rib_frame():
    ...
    cfg.step7_diepte=diepte.iloc[0,0]+diepte.iloc[0,1]*2+cfg.plank_dikte
    #print(breedte)
    
    #BREEDTE
    xmax = ribmax.loc[ribmax['xloc'].idxmax()]
    xmax = xmax[3]
    cfg.step7_breedte=xmax*2+cfg.balk_dikte
    ...
    cfg.step7_hoogte = hoogte + cfg.poot_hoogte + cfg.plank_dikte*2

    cfg.step7_Ox=0.
    cfg.step7_Oy=0.
    cfg.step7_Oz=cfg.step7_hoogte/2.
    ...
    for row in range(rows):
        ...
        B=balk.construct(x0,y0,z0,l,w,h,xa,ya,za)
        cfg.step7_ribben_vert.append(B)
        pa=B[-2]
        pb=B[-1]
        arrowlist.append([pa,pb,l,w,h])
    ...
    for row in range(rows):
        ...
        B=balk.construct(x0,y0,z0,l,w,h,xa,ya,za)
        cfg.step7_ribben_horiz.append(B)
...
def build_arrow(arrowlist,arrowlist2):
    ...
    for a in range(len(arrowlist2)):
        ...
        '''  
        if a == 0:        
            cfg.step7_cam22=vector(x0,y0,z0)
        
        if a == middleIndex:
            cfg.step7_cam21=vector(0,0,abs(arrowlist[a][0][2]-arrowlist[a][1][2])/2.)
        '''          
        A=get_arrow(x0,y0,z0,x1,y1,z1,x2,y2,z2,thickness)
        cfg.step7_arrow.append(A)
...
def build(path,lang):
    ...
    table_builder.latex([cfg.bovenkant],path,lang,7)