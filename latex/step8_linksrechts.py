...
def get_rib_frame():
    ...
    cfg.step8_diepte=diepte.iloc[0,0]+diepte.iloc[0,1]*2+cfg.plank_dikte
    #print(breedte)
    
    #BREEDTE
    xmax = ribmax.loc[ribmax['xloc'].idxmax()]
    xmax = xmax[3]
    cfg.step8_breedte=xmax*2+cfg.balk_dikte
    ...
    cfg.step8_hoogte = hoogte + cfg.poot_hoogte + cfg.plank_dikte*2

    cfg.step8_Ox=0.
    cfg.step8_Oy=0.
    cfg.step8_Oz=cfg.step8_hoogte/2.
    ...
    for row in range(rows):
        ...
        B=balk.construct(x0,y0,z0,l,w,h,xa,ya,za)
        cfg.step8_ribben_vert.append(B)
        pa=B[-2]
        pb=B[-1]
        arrowlist.append([pa,pb,l,w,h])
    ...
    for row in range(rows):
        ...
        B=balk.construct(x0,y0,z0,l,w,h,xa,ya,za)
        cfg.step8_ribben_horiz.append(B)
...
def get_rechts():
    ...
    for row in range(rows):
        ...
        B=balk.construct(x0,y0,z0,l,w,h,xa,ya,za)
        cfg.step8_zeiderechts.append(B)
...
def build_arrow(arrowlist,arrowlist2):
    ...
    for a in range(len(arrowlist2)):
        ...
        z0=arrowlist[-a-2][0][2] #+ arrowlist[a][4]/2
        ...
        z1=arrowlist[-a-2][0][2] #+ arrowlist[a][4]/2
